        ...
        <el-form-item prop="captcha">
          <div class="captcha-container">
            ...
            <div class="login-code" width="100%" @click="refreshCaptcha">
              <!--验证码组件-->
              <CaptchaDisplay :identifyCode="captchaText" /> 
            </div>
          </div>
        </el-form-item>
 ...
 * 
 * 功能特性：
 * - 用户信息收集（手机号、密码、真实姓名、身份证号等）
 * - 图形验证码验证
 * - 短信验证码发送和验证
 * - 表单数据验证
 * - 用户协议和隐私政策确认
 * 
 * @author Railway System Team
 * @version 1.0.0
 */
...
import { register, sendCode as sendSms, getCaptcha } from '@/api/auth'
import type { RegisterRequest, UserInfo, SmsResponse } from '@/api/auth'
...
/**
 * 验证手机号是否有效
 * @returns {boolean} 手机号是否符合中国大陆手机号格式
 */
...
const refreshCaptcha = async () => {
  try {
    ...
    captchaText.value = response.data
  } catch (error) {
